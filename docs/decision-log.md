...
## 2026-05-14 — [iOS] XcodeGen for project file management

**Decision:** the iOS app's Xcode project (`apps/ios/PulseCoffeeApp.xcodeproj`) is generated by [XcodeGen](https://github.com/yonaskolb/XcodeGen) from a declarative `apps/ios/project.yml` spec. The generated `.xcodeproj` is **gitignored**; `project.yml` is the sole source of truth. Developers run `make project` (from `apps/ios/`) after pulling to regenerate the project file locally.

**Considered:**

| Option | Pros | Cons | Decision |
|---|---|---|---|
| Commit the `.pbxproj` directly, no generator (Xcode default) | Zero install friction | Famously unmergeable. Every cosmetic Xcode interaction (group reorder, scheme tweak, build setting touched in GUI) produces line-noise diffs that resist meaningful review. Across AI-assisted multi-session work the conflict rate approaches 100%. | Rejected |
| Commit BOTH `project.yml` AND the generated `.xcodeproj` | Reviewers can open project without installing XcodeGen | Two artifacts can drift silently: every `project.yml` edit requires regenerating + committing the `.xcodeproj` and no automated check enforces it. The drift mode ("works on my machine because I regenerated locally; broken for everyone else") is more confusing than a one-time install step. | Rejected |
| **Commit only `project.yml`; gitignore `.xcodeproj`** | Single source of truth. No merge conflicts in the project file ever. Forces every contributor to regenerate locally, which means stale local `.xcodeproj` files can't ship to others. | One-time `brew install xcodegen` onboarding step. | **Chosen** |
| Tuist (Swift-DSL alternative) | Typed manifests, generate-on-demand workspaces, caching | More complexity than the project warrants at Phase 1 scale (single app, no extensions, no watch app). Pays back at larger scale. | Rejected for Phase 1; worth revisiting if a watch app or app extension lands. |

**Why this:**

- The `.pbxproj` is genuinely unmergeable in any meaningful sense — even cosmetic Xcode changes produce diff churn that masks substantive changes in review. Across multi-session AI work, the conflict rate becomes a meaningful tax on every iOS commit.
- XcodeGen's `project.yml` is a YAML file: easy to review, easy to diff, easy to merge. Adding a new source file or moving one between targets is a one-line edit; the generated `.pbxproj` rebuilds itself.
- It is the de facto industry standard for serious iOS projects with multiple contributors (used by Lyft, Slack, Spotify, Yelp, and most open-source iOS apps with active community contribution).
- Onboarding cost: `brew install xcodegen` (one command, one machine, one time). Documented in `apps/ios/README.md` and `apps/ios/Makefile`.

**Workflow:**

```
project.yml  ──┐
               ├─ make project ─→  PulseCoffeeApp.xcodeproj (gitignored)
.gitignore  ──┘
```

After any `project.yml` change, run `make project`. Xcode picks up the regenerated project file via its built-in file-change watcher.

**Followed-up items:**

- A CI step that runs `xcodegen generate` and asserts `git status` is clean would prevent silent drift between `project.yml` and the developer-local `.xcodeproj`. Not landed today because iOS CI is deferred to the DevOps phase (separate decision-log entry).
- A pre-commit hook doing the same thing is the local equivalent. Deferred to whoever sets up shared dev tooling.

**Companion changes in this commit:**

- `apps/ios/project.yml` — XcodeGen spec covering both the app target and the unit-test bundle, both schemes, push-notifications entitlement, iPhone-only TARGETED_DEVICE_FAMILY, iOS 16 deployment target, Automatic signing with team `3828C7WU89`, bundle ID `com.pulsecoffee.app`.
- `apps/ios/Makefile` — `install` / `project` / `build` / `test` / `clean` targets. `install` is idempotent (skips if `xcodegen` is already on PATH).
- `apps/ios/PulseCoffeeApp/` — minimal SwiftUI scaffold: `@main` app with placeholder Sentry-init comment (commit #2 lands the real call), `ContentView` with a Debug-only yellow banner displaying the API base URL (prevents the "why isn't this working" debug session when localhost / tunnel / staging gets confused), entitlements file with `aps-environment=development`, asset catalog placeholders for `AppIcon` + `AccentColor`, preview-content directory.
- `apps/ios/PulseCoffeeAppTests/PulseCoffeeAppTests.swift` — one trivial scaffold test (`test_scaffold_imports_app_module`) confirming the test bundle compiles and links against the app target. Real coverage starts in commit #3 (`APIClient`, `Keychain`, Codable models).
- Root `.gitignore` — iOS / Xcode block (gitignored `.xcodeproj`, DerivedData, xcuserdata, Carthage/Pods/fastlane defensive blocks). `Package.resolved` is NOT gitignored (it's the SPM dependency lockfile and must ship with the app for reproducible builds).

**Deferred to later commits:**

- `aps-environment` is hardcoded to `development` for now. TestFlight builds require `production`. Commit #11 (TestFlight prep) wires a `$(APS_ENVIRONMENT)` build setting so the Debug/Release split is automatic.
- Apple Pay capability is NOT in the entitlements yet. It requires the Merchant ID (`merchant.com.pulsecoffee.app`) which the CTO is creating in parallel. Lands in commit #7 (checkout) when the dependency is satisfied.
- ATS exception for `http://localhost` lands in commit #3 with the `APIClient`. Without it, the Debug build cannot talk to a local backend.
- Strict-concurrency mode is set to `minimal` for the scaffold. Tighten to `complete` after commit #2 SPM dependencies (Stripe, Sentry, PostHog) are integrated — third-party libraries occasionally lag on Sendable conformance and we want stable baselines before turning on the strictest mode.

**Pre-flight findings worth flagging (not blocking this commit):**

- `docs/ai-onboarding/ios.md` rule #10 requires every endpoint call to reference a contract file in `docs/contracts/`. **That directory does not exist.** Recommendation: treat the backend's `@ApiProperty`-decorated DTOs + the auto-generated Swagger UI at `/api/docs` as the authoritative contract source, and supersede the contracts-dir requirement with a follow-up decision-log entry. Maintaining a parallel `docs/contracts/` would invite drift between the directory and the backend code. Decision pending CTO input — not blocking the scaffold.
- `docs/ai-onboarding/ios.md` lists `GET /loyalty/my` for the Loyalty screen (commit #10). The backend loyalty module is "Next" status per `apps/api/README.md`. Commit #10 will need either a Loyalty stub view, a "coming soon" placeholder, or a coordination point with the backend chat to ship the endpoint before commit #10 lands.
- The hardcoded FeatureFlags pattern from the CTO's approval is correct for genuinely client-side flags (AI recommendations, subscription pass). For `scheduledOrdering` specifically, the gate is already implemented server-side via `location_settings.scheduled_ordering` — iOS should derive the UI state from the location's settings (returned by `GET /locations/:id`), not from a static flag. Otherwise the user picks a scheduled time, hits the server, and gets a rejection. Will wire correctly in commit #5 (menu / locations).


---

## 2026-05-14 — [iOS] Contracts source of truth: backend DTOs + Swagger

**Decision:** the iOS app does NOT maintain a parallel `docs/contracts/` directory. The backend NestJS DTOs (decorated with `@ApiProperty`) plus the auto-generated Swagger UI at `/api/docs` are the authoritative source of truth for every API shape iOS consumes. Each iOS Codable model carries a header comment naming the backend DTO file it mirrors; that comment is the cross-reference for "what does this map to, who would change it."

**Considered:**

| Option | Pros | Cons | Decision |
|---|---|---|---|
| Maintain `docs/contracts/` parallel to backend DTOs (the literal reading of `docs/ai-onboarding/ios.md` rule #10) | Single docs surface for iOS to consume without reading TypeScript | Two artifacts drift silently. Every backend DTO change requires updating the doc file. Reviewers cannot tell whether the doc is current with the backend. The Swagger UI is already generated from the same source — duplicating it is busywork. | Rejected |
| **Treat backend `@ApiProperty` DTOs + auto-generated Swagger as the contract** | Single source of truth. Backend changes propagate automatically. No drift risk. Swagger UI doubles as live documentation for ad-hoc verification. | iOS engineers must read TypeScript DTOs (or browse Swagger) rather than a single markdown file. | **Chosen** |

**Why this:**

- The backend already has the contract — it's the DTO file with `@ApiProperty` decorators. Building a parallel markdown copy would be a second source of truth with no enforcement that the two stay in sync.
- The Swagger UI at `http://localhost:3000/api/docs` is regenerated from the same decorators on every backend restart and is always current with the running backend. iOS developers cross-reference Swagger during implementation.
- Static cross-references in iOS code make backend-change impact locally discoverable. A `git grep` for the DTO file path in `apps/ios/` surfaces every Codable that needs updating.

**iOS implementation pattern:**

Every Codable model has a header comment naming the backend DTO it mirrors:
...
The grep pattern when a backend file changes:

```bash
git grep "apps/api/src/modules/auth/dto/register.dto.ts" apps/ios/
```

…surfaces every iOS impact site immediately.

**Field naming on iOS — explicit `CodingKeys`, not `convertFromSnakeCase`:**

The backend returns snake_case payloads (e.g., `access_token`, `full_name`). iOS uses explicit `CodingKeys` per Codable rather than `JSONDecoder.keyDecodingStrategy = .convertFromSnakeCase`. Two reasons:

1. Explicit keys document each field mapping at the call site. A future engineer scanning the Codable sees the exact wire name immediately.
2. A backend field rename surfaces as a compile-time signal when CodingKey is updated — the strategy-based approach would silently keep working with `nil` defaults.

**Supersedes:**

This entry **supersedes** `docs/ai-onboarding/ios.md` rule #10, which directed iOS engineers to reference `docs/contracts/` and stop if a contract was missing. That rule was authored before the iOS chat picked up the work; the parallel-docs approach it implies adds friction without commensurate benefit.

A follow-up commit can update `docs/ai-onboarding/ios.md` rule #10 inline to reflect this decision. Not landed in this commit — that's downstream documentation hygiene that belongs in its own `docs:` commit; bundling it here would mix the scaffolding scope with onboarding-doc edits.

**What this does NOT remove:**

- Strict typing on iOS Codables remains a hard rule. `JSONDecoder` failures must surface a clear error to the developer (and Sentry breadcrumb to the app), never silently produce zero values.
- Sentry breadcrumbs on every API call (per `docs/ai-onboarding/ios.md` definition-of-done) remain in force.
- The verification step before implementing a new endpoint: read the controller, read the DTO, optionally hit Swagger UI to confirm shape.

---

## 2026-05-14 — [iOS] Loyalty view ships placeholder copy in Phase 1

**Decision:** the Phase 1 `LoyaltyView` (commit #10) ships a placeholder screen — title + "rewards launching soon" copy + back-to-menu CTA. **No mocked points data, no fake tier badges.** The backend loyalty module is "Next" status per `apps/api/README.md` and the `GET /loyalty/my` endpoint that `docs/ai-onboarding/ios.md` references does not exist yet. Rather than block iOS launch on backend timeline, ship an honest placeholder; the swap to real loyalty when the endpoint lands is a single commit.

**Considered:**

| Option | Pros | Cons | Decision |
|---|---|---|---|
| Ship `LoyaltyView` wired to `GET /loyalty/my` | Real feature on launch | Endpoint does not exist. Would block iOS commit #10 on backend timeline. iOS launch should not depend on a feature that won't be visible to users for the first weeks anyway. | Rejected |
| Ship `LoyaltyView` with mocked points/tier data | Looks impressive in screenshots | Lies to users. Points "earned" against the mock will vanish when real loyalty ships, eroding trust at exactly the moment we're trying to build it. | Rejected |
| **Ship placeholder copy** | Honest. Tiny swap when backend ships loyalty. No misleading data. | Phase 1 launch UI is slightly less impressive on the loyalty tab. | **Chosen** |
| Remove the Loyalty tab from Phase 1 entirely | Smallest scope | Loyalty is in the Phase 1 feature roadmap (spec page 24, features #9–#10). Removing the tab reshapes the navigation shell, costing more than the placeholder. | Rejected |

**Why this:**

- "Loyalty system foundation" is in the spec as Phase 1 (features #9–#10), but the backend endpoint is not built. The iOS chat is not authorized to extend the backend; coordinating an endpoint ship before commit #10 risks slipping launch on a non-critical-path feature.
- Mocked data is corrosive to user trust. "I earned 50 points!" that resets to zero when real loyalty ships is worse than a "rewards on their way" placeholder from day one.
- The swap is small: replace `PlaceholderLoyaltyView` with a real `LoyaltyView` wired to `GET /loyalty/my`. A future decision-log entry on the real Loyalty view will reference this entry as its predecessor.

**Placeholder copy (subject to design polish at implementation time):**

- Title: "Loyalty Rewards"
- Body: "Earn points with every order. Rewards program launching soon!"
- CTA: "Continue Browsing" → returns to the Menu tab
...
4. **`badge_type` ships as plumbing only.** Nullable column — all items default `null` via the migration; the seed does not set `badge_type`, preserving any operator-assigned badge on re-seed. Exposed in `PublicMenuItem`, rendered monochrome on iOS only when present. No fake badges, no social-proof numbers.
