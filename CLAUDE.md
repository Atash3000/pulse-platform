...
- **`/new-issue <type>: <short title>`** — wraps `gh issue create` and hands back a CLAUDE.md §7-compliant branch name to use for the work. Example: `/new-issue feat: orders tab icon for iOS nav` → creates issue #N and returns `feat/N-orders-tab-icon-for-ios-nav` plus the `git switch -c` command. Does not auto-create the branch; surfaces the suggestion so you start work intentionally. Definition: `.claude/commands/new-issue.md`.
