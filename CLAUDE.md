...
- **`/review-sec [pr | branch | git-range]`** — security-focused review of a branch, PR, or git ref range. Resolves the scope (default: current branch vs `main`), filters the diff to security-scoped paths, then delegates to the `security-reviewer` sub-agent. Enforces Stripe-webhook signature + raw-body discipline, event-id dedup, JWT secret hygiene (env-only, separate access/refresh secrets), bcrypt rounds, throttle coverage on public endpoints, ValidationPipe / class-validator coverage, ATS narrow scope, Keychain access class, `SentryRedactor` scrub-list completeness, and the auth-perimeter audit (every controller's guard placement). Read-only. Definition: `.claude/commands/review-sec.md`.
...
- **`security-reviewer`** — read-only specialist for auth / JWT / Stripe webhook signature verification / secret hygiene / input validation / rate limiting / iOS token storage. Invoked by `/review-sec`. Enforces webhook-signature + raw-body discipline, webhook event-ID dedup before side effects, separate access/refresh JWT secrets, bcrypt rounds ≥ 10, throttle coverage on public endpoints, ValidationPipe with `whitelist + forbidNonWhitelisted`, class-validator decorators on every DTO field, ATS exception narrowly scoped to `localhost`, Keychain `kSecAttrAccessibleWhenUnlocked`, `SentryRedactor` scrub-list keeps up with what `APIClient` sends, and audits the controller perimeter (every `*.controller.ts` route's guard placement vs. expected-public list). Uses `Known Gap` tier for spec-implied hardening the codebase hasn't done yet (refresh-token rotation, CORS allowlist, helmet). Definition: `.claude/agents/security-reviewer.md`. Same restricted tool list as `payment-reviewer` — read-only by design.
