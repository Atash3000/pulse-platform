...
The v4 Menu screen — header + a sticky `CategoryTabBar` **category selector** (one category shown at a time) + that category's spotlight section — plus the shared abstract drink-symbol system used in spotlight hero cards and list rows. Every category uses the `spotlight` layout.
...
| File | Purpose |
|---|---|
| `MenuView.swift` | Screen entry point. Hides the system nav bar, renders the custom topbar (status dot + location name + cart) and the sticky `CategoryTabBar`, and switches on `MenuViewModel.state` to show loading / failed / empty / loaded. The loaded body renders **only the selected category's** `SpotlightSection`; tapping a tab swaps which category is shown. The scroll's bottom inset clears the 72pt `PulseTabBar` and holds a `BrandFooter` wordmark. Smart-add wired here via `handleAdd(_:)` calling `MenuListRow.canInstantAdd(_:)`. |
| `MenuViewModel.swift` | `@MainActor` view-model. Bootstraps `LocationSummary` + `Menu`, holds `state`. No temperature filtering. The selected category drives which section renders. |
...
| `SpotlightSection.swift` | Renders a category as **hero card + up to 3 sub-hero cards + a vertical `MenuListRow` list** for the rest. Used by **all** categories. Hero is the `featured` item (else the first); the sub-hero/vertical split follows the backend `sort_order` (items arrive pre-ordered). Hero/split logic in static `hero(in:)` + `nonHeroItems(in:hero:)` + sub-hero/vertical helpers so the rules are independently testable. |
...
- **Active-pill slide animation.** The category selector's active pill is a *single* dark-espresso `Capsule` drawn only behind the selected tab and tagged with `matchedGeometryEffect(id:in:)`. When the selected category changes (on tap) SwiftUI interpolates that one pill's frame from the old tab to the new one, so it slides instead of snapping; the tab labels cross-fade taupe ↔ cream as it passes. The slide is a spring (`response: 0.32, dampingFraction: 0.72`) applied via `withAnimation`. **Reduce Motion** is honored — `@Environment(\.accessibilityReduceMotion)` collapses the animation to instant (`nil`) so the control degrades to a hard switch, never a removed feature (Golden Rule #17 spirit: motion is non-essential polish).
