...
| `TemperatureToggle.swift` | All / ☕ Hot / ❄ Iced segmented pill + the `TemperatureFilter` enum bound to `MenuViewModel.selectedTemperature`. View subviews are extracted (`segment(for:)`, `segmentBackground(isActive:)`, `trackBackground`) so Swift's type-checker doesn't trip. The active dark-espresso pill **slides** between segments via `matchedGeometryEffect` + a spring (see "Toggle slide animation" below). |
...
- **Toggle slide animation.** The temperature toggle's active pill is a *single* dark-espresso `Capsule` drawn only behind the selected segment and tagged with `matchedGeometryEffect(id:in:)`. On a selection change SwiftUI interpolates that one pill's frame from the old segment to the new one, so it slides instead of snapping; the segment labels cross-fade taupe ↔ cream as it passes. The slide is a spring (`response: 0.32, dampingFraction: 0.72`) applied via `withAnimation` on tap. **Reduce Motion** is honored — `@Environment(\.accessibilityReduceMotion)` collapses the animation to instant (`nil`) so the control degrades to a hard switch, never a removed feature (Golden Rule #17 spirit: motion is non-essential polish).
