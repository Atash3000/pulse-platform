...
Bottom tab bar shown to everyone (guests and signed-in users). Four tabs (Home / Menu / Orders / Rewards), patterned after Luckin Coffee's layout. Account is **not** a tab — it is reached via the top-right `AccountAvatarButton` on the Home screen (first-name initial when signed in, person glyph when guest), which presents `AccountView` as a sheet. See `docs/superpowers/specs/2026-06-06-account-avatar-home-design.md` and the 2026-06-06 decision-log entry.

```
ContentView                       ← root auth gate
└── MainTabView                   ← this folder — shown to guests and signed-in users
    ├── HomeView          (Home)      placeholder — hosts AccountAvatarButton → AccountView sheet
    ├── MenuView          (Menu)      real screen — see Features/Menu/
    ├── OrdersView        (Orders)    placeholder
    └── RewardsView       (Rewards)   placeholder

AccountView                       ← not a tab; presented as a sheet from Home's avatar
```
...
| File | Purpose |
|---|---|
| `MainTab.swift` | Typed enum (`home`, `menu`, `orders`, `rewards`) with title + SF Symbols. One source of truth for the tab bar, deep-links, and analytics. |
| `MainTabView.swift` | The tab host plus custom bottom bar. Menu is the launch tab for signed-in users; guests land on Home. |
| `Placeholders.swift` | `HomeView`, `OrdersView`, `RewardsView`, `AccountView` — minimal stubs (title + icon + caption). `AccountView` is presented as a sheet from Home, not as a tab. Promote to their own files when real content lands. |
...
Account is no longer a tab. It is reached via the `AccountAvatarButton` on Home, which presents `AccountView` as a sheet (`guest: WelcomeView cold-open / join surface; logged-in: placeholder + Sign Out button`). The real profile screen (payment methods, order history, edit profile) lands later.

## Auth-state routing

`ContentView` now opens `MainTabView` for **both** logged-in and logged-out users — the difference is the initial tab.

| Auth state | `initialTab` | Account entry |
|---|---|---|
| `.loggedOut` | `.home` | `AccountAvatarButton` on Home presents `AccountView` → `WelcomeView` (Features/Auth/WelcomeView.swift) as a sheet. Sheets present `LoginView` / `RegisterView`. The Menu tab stays browsable for guests. |
| `.loggedIn` | `.menu` | `AccountAvatarButton` on Home presents `AccountView`'s placeholder, until the real profile screen lands. |

`AccountView` reads `AppState.authState` directly and branches between `WelcomeView` and the placeholder, so the avatar sheet shows the right content in either tree.

Note that auth changes do trigger a full re-render: `ContentView`'s `switch` re-instantiates `MainTabView` with the new `initialTab` when `authState` flips, tearing down the old tab tree. A guest who signs in therefore lands on **Menu** (the new tree's `initialTab`), not back on Home — and that teardown is what clears the in-memory cart / view-model state on logout.

## Follow-ups

- **Guest-gate the tabs.** With `ContentView` now letting logged-out users into `MainTabView` (landing on Home), the Menu tab works (public endpoint), but Cart / checkout / Orders / Home aren't yet aware of guest state. Next commits should: (a) replace the Cart's "Checkout" button with a "Sign in to checkout" CTA when guest, (b) show a "Sign in to see your orders" empty state in `OrdersView`, (c) hide or stub the streak / rewards strips on the Home tab when guest. Each is one focused commit per §1.6.
...
- **Cart icon placement.** The cart button currently lives in the v4 Menu topbar (right side, where the HTML `nav-profile` chip sits). The HTML mockup uses an avatar there; iOS keeps the cart because it's a critical commerce affordance and the profile entry is the `AccountAvatarButton` on Home. Once a richer customer-profile UI ships, revisit whether the cart belongs tab-bar-adjacent (Luckin floats a checkout bar above the tab bar).
- **Lazy tab mounting before real Home / Orders / Rewards data work.** `MainTabView` keeps all four tab roots alive and hides inactive tabs with opacity so the custom bar can own selection. That is fine while only Menu does real work, but before other tabs add API fetches or polling we should lazy-mount inactive tabs or gate their `.task` bodies on the selected tab.
...
- **Repeat-tap native tab behavior.** Native `TabView` can pop the selected tab to root or scroll a list to top when the already-selected tab is tapped. The custom bar intentionally defers this until Home / Orders have deeper navigation stacks.
