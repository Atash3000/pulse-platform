...
- **Icons are SF Symbols plus Pulse marks.** Home uses two template-rendered SVG layers from `Assets.xcassets/PulseHomeMark.imageset` and `Assets.xcassets/PulseHomeLeafAccent.imageset`; Menu uses the Pulse `P` cup SVG from `Assets.xcassets/PulseCupMark.imageset`; Orders composes a template-rendered bag layer from `Assets.xcassets/PulseOrdersMark.imageset` with a separate cup-status layer from `Assets.xcassets/PulseOrdersCupState.imageset`; Account uses the template-rendered profile mark from `Assets.xcassets/PulseAccountMark.imageset`. The Home base layer follows the active/inactive icon color, while its matcha curl uses `AppTheme.Colors.tabIconMatchaAccent` only when selected and taupe when inactive. Account and Orders also follow the shared template color logic. Menu remains original-rendered for now so its supplied logo gradients survive at tab size. A third-party illustrated set (Phosphor, Lucide, Iconoir — all MIT-licensed) is a follow-up if/when product wants a broader custom icon set. See `docs/decision-log.md` entry "[iOS] SF Symbols over third-party icon library for tab bar" for the full reasoning.
