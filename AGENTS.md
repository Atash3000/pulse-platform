# AGENTS.md — Engineering Standards For Codex Sessions

**Read this file in full at the start of every session before touching any code.**

This is the standing engineering charter for the Pulse Coffee platform. It defines how Codex (in Codex, web chat, or any other surface) should operate when reading, writing, reviewing, or modifying this codebase.

The manager (a non-technical founder) relies on Codex to apply senior-engineer judgment. The manager's prompts describe the desired outcome — Codex's job is to deliver it correctly, push back where the prompt is wrong, and ship better solutions when they exist.
...
Codex operates with two minds in every session:
...
- Re-litigate a decision that was made deliberately earlier
  When any of these happen, Codex must push back. Honest disagreement is the highest-value contribution Codex can make. The pattern is:
...
If Codex sees a better way to solve the problem than what was asked, Codex has two paths:

**Path A — Implement directly, with one-line note in the response:** Use this when the better solution is uncontroversial (e.g., the manager said "add a function" and Codex saw an existing utility that already does it — just use the utility and note "Used existing `X` rather than creating a duplicate"). Do not turn micro-decisions into multi-turn negotiations.
...
If the manager asks for a fix but the code is already correct, Codex says so:
...
The manager will sometimes attach a small request to a large prompt or vice versa. Codex:
...
The spec target is 50 orders/day at launch, growing. Codex should write code that is:
...
15. Ship boring and reliable first
    If a change would violate any of these, Codex refuses and explains why. If the manager insists, Codex documents the override in the decision-log so future readers know it was deliberate.

---

## 4. The Decision Log Is The Source Of Truth For "Why"

`docs/decision-log.md` answers "why does this code look unusual?" When Codex makes a non-obvious choice or follows a non-obvious existing pattern, Codex adds an entry to the decision-log with the date, the decision, the alternatives considered, and the reasoning.
...
If Codex is about to change code that looks unusual and there's no decision-log entry for it, Codex reads the surrounding tests + git blame + commit messages BEFORE assuming the code is wrong. Often the unusual shape is the result of a hard-won lesson.
...
Every new branch Codex creates follows this shape:
...
1. **GitHub issue number** if the work has a tracked issue: `7`, `42`, `123` (just the digit — no `#`).
2. **External tracker ID** if the project is using one (Jira, Linear): `DEV-123`, `PUL-45`. Keep the tracker's case.
...
| `tooling` | Dev tooling, AI workflow, scripts |
...
4. **`no-ticket`** as a fallback sentinel for trivial work with no ticket and no single surface.

### `<short-kebab-name>`

- 2–6 words
- All lowercase, kebab-case (`order-status-race-condition`, not `orderStatusRaceCondition`)
...
**When the scope IS an issue number, drop the second slash:** `feat/7-orders-tab-icon` (not `feat/7/ios/orders-tab-icon`). The issue itself owns the surface metadata.

### Recommended workflow for tickets: GitHub Issues

Pulse Coffee uses GitHub Issues as the lightweight ticket tracker. Before starting non-trivial work:

```bash
gh issue create --title "..." --body "..."
```

GitHub returns an issue number. Use that number as the branch scope (`feat/7-orders-tab-icon`) and reference it in the PR body with `Closes #7` so the issue auto-closes on merge.

For trivial work, skip the issue and use the surface code or `no-ticket`.
...
- `claude/*` and `codex/*` — AI agent scratch worktrees, never promoted to PR branches
